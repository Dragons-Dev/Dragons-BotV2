...
class ExtensionsCommands(commands.Cog):
    ...
    @discord.option(
        autocomplete=folder, name="folder", description="Select the Folder", required=True
    )
    @discord.option(
    ...
    async def activate_extension(self, ctx: discord.ApplicationContext ,folder: str, extension: str):
        ...
        if path not in extension_store:
            return await ctx.response.send_message(f"The selected extension {path} doesn't exist", ephemeral=True, delete_after=5)
        
        extension_store[path] = True
        with open("./assets/disabled.json", "w") as f:
            json.dump(extension_store, f, indent=4)
        
        await ctx.response.send_message(f"✅ `{folder}.{extension}` has been activated.", ephemeral=True, delete_after=5)
    ...
    async def deactivate_extension(self, ctx: discord.ApplicationContext ,folder: str, extension: str):
        ...
        await ctx.response.send_message(f"🚫 `{folder}.{extension}` has been deactivated.", ephemeral=True, delete_after=5)
